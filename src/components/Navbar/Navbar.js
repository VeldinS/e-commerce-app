...
function Navbar() {
    ...
    return (
        <nav className={'flex flex-row justify-between items-center px-4 lg:px-48 pt-4 pb-4 bg-light-gray absolute top-0 w-full z-50 overflow-hidden'}>
            ...
            <div className="lg:hidden"> {/* Conditionally render hamburger icon on small screens */}
                <button onClick={toggleMenu}>
                    {isMenuOpen ? <XIcon className="h-6 w-6 text-black" /> : <MenuIcon className="h-6 w-6 text-black" />}
                </button>
            </div>
        </nav>
    );
