...
function SellOfDay() {
    ...
    useEffect(() => {
        fetchSpecificProducts('sell_of_day')
            .then(setProduct)   // Update state with the fetched data
            .catch(err => {
    ...
    return (
        <section className={'bg-terracotta h-auto py-24 xl:px-48 lg:px-24 px-6 flex relative lg:flex-row flex-col justify-between lg:items-start items-center gap-8 overflow-hidden'}>
            <div className={'lg:w-3/5 w-full flex flex-col lg:items-start items-center justify-start lg:gap-6 gap-2'}>
                ...
                {product.map((product) => (
                    <div key={product._id} className={'flex flex-col lg:items-start items-center justify-center lg:gap-8 gap:2 lg:w-3/5 w-full'}>
                            <p className={'font-roboto text-black text-xl lg:text-start text-center leading-normal'}>
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin egestas orci {product.name}
                            </p>
                        <Countdown />
                        <Link href={`/products/${product._id}`} link={`/products/${product._id}`}>
                            <ButtonPrimary text={"Check it Out!"} link={`/products/${product._id}`} />
                        </Link>
                    </div>
                ))}
            </div>
